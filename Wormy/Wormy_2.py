...
# Definiranje nov event za kreiranje na nov crv po 45 sekundi
TIME_EVENT = pygame.USEREVENT


def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, direction2
...
def runGame():
    global FPS
    # Postavuvanje timer
    pygame.time.set_timer(TIME_EVENT, 45000)
    # Set a random start point.
    startx = random.randint(5, CELLWIDTH - 6)
    starty = random.randint(5, CELLHEIGHT - 6)


    # Zadavanje pochetni koordinati za vtoriot crv
    ...
    # Zadavanje direkcii i koordinati za vtoriot crv
    ...
    # Kreiranje na kopchinjata za otpochnuvanje na nova igra ili otkazuvanje
        ...
        for event in pygame.event.get(): # event handling loop
            if event.type == MOUSEBUTTONUP:
                if NEW_RECT.collidepoint(event.pos):
                    generateNewGame()  # clicked on Reset button
            ...
            if event.type == QUIT:
                terminate()
            # Ako se sluchi eventot, da se icrta vtoriot crv i da mu se zadade direkcija na dvizenje
...
#  Kreiranje nova funkcija na iscrtuvanje za noviot crv
