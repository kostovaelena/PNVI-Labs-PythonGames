...
# Zgolemuvajne na delot na kamerata
...
# Dodavanje dve posebni konstanti za cameraslack horizontalno i vertikalno
CAMERASLACK_X = 80
CAMERASLACK_Y = 60
...
# Kolkava treba da bide goleminata na ververichkata za da izgubi
LOSTSIZE = 5
...
# Baranje 3.1
            if sObj['bounceheight'] > 0:
                sObj['bounce'] += 1
                if sObj['bounce'] > sObj['bouncerate']:
                    sObj['bounce'] = 0 # reset bounce amount
            elif sObj['bounceheight'] < 0:
                sObj['bounce'] -= 1
                if sObj['bounce'] < sObj['bouncerate']:
                    sObj['bounce'] = 0 # reset bounce amount
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        del squirrelObjs[i]
                    # Ako ververichkata na igrachot e pomala od onaa so koja se sudrile, treba da se namali
                    else:
                        playerObj['size'] -= int((sqObj['width'] * sqObj['height']) ** 0.2) + 1
                        del squirrelObjs[i]
                    ...
                    # Igrachot gubi ako dostigne golemina pomala od LOSTSIZE
                    if playerObj['size'] < LOSTSIZE:
                            winMode = False # turn off "win mode"
...
def makeNewSquirrel(camerax, cameray):
    ...
    # Baranje 3.1
    c = random.randint(0, 1)  # 0 za downwards, 1 za upwards
    if c == 0:
